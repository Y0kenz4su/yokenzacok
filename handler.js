...
module.exports = {
    async handler(chatUpdate) {
        ...
        global.wm = '⁺◛˖Yokenza Gans'
        global.colong1 = 'Ciss 📸'
        global.colong2 = 'YzMods'
        global.kontak2 = [
        ['6288226749699', 'Yokenza', '𝙿𝚎𝚗𝚐𝚎𝚖𝚋𝚊𝚗𝚐', 'Nyolong Sc Trus Di Kembangin Lagi', true],
        ['6288226749699', await this.getName('6288215114685@s.whatsapp.net'), 'No Gwe Satunya', true], 
        ]
    ...
    async participantsUpdate({ id, participants, action }) {
        ...
        switch (action) {
            ...
            case 'demote':
                if (!text) text = (chat.sDemote || this.sdemote || conn.sdemote || '@user ```Akwowkwo Ga Jadi Admin```')
    ...
    async groupsUpdate(groupsUpdate, fromMe, m) {
        ...
        for (let groupUpdate of groupsUpdate) {
            ...
            if (groupUpdate.desc) text = (chats.sDesc || this.sDesc || conn.sDesc || '```Deskripsi Telah Di Ubah Menjadi```\n@desc').replace('@desc', groupUpdate.desc)
            if (groupUpdate.subject) text = (chats.sSubject || this.sSubject || conn.sSubject || '```Subjek Telah Di Ubah Menjadi```\n@subject').replace('@subject', groupUpdate.subject)
            if (groupUpdate.icon) text = (chats.sIcon || this.sIcon || conn.sIcon || '```Icon Telah Di Ubah Menjadi```').replace('@icon', groupUpdate.icon)
            if (groupUpdate.revoke) text = (chats.sRevoke || this.sRevoke || conn.sRevoke || '```Link Grup Telah Di Ubah Menjadi```\n@revoke').replace('@revoke', groupUpdate.revoke)
            ...
            if (groupUpdate.restrict == true) text = (chats.sRestrictOn || this.sRestrictOn || conn.sRestrictOn || '```Group telah dibuka untuk semua orang!')
            if (groupUpdate.restrict == false) text = (chats.sRestrictOff || this.sRestrictOff || conn.sRestrictOff || '```Group telah di atur hanya admin!')
            //console.log('=============\n\ngroupsUpdate \n\n============\n\n' + await groupUpdate)
            if (!text) continue
